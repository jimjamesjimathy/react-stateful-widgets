...
export default function Programmers() {
  ...
  const getNameOfFeatured = () => {
    ...
    const oneOfAwesome = programmers.find(element => element.id === featureId);
    return oneOfAwesome.name;
  };
      ...
      <div className='programmers'>
        {
          ...
          programmers.map(developer =>
            <div className='programmer' key={developer.id}>
              {developer.name} <button onClick={() => {setFeatureId(developer.id) /* in here set the featured id to be dev.id */ }}>Feature</button>
            </div>
          )
        }
      </div>
